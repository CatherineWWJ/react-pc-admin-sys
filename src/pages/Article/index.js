...
const Article = () => {
   ...
   * 解释：避免性能损失
   * 放在useEffect外面：每次组件更新都会重新进行函数初始化  这本身就是一次性能消耗
   * 放在useEffect内部：只会在依赖项发生变化的时候  函数才会进行重新初始化
   */
  // 获取文章列表
  useEffect(() => {
  ...
  const columns = [
    {
      ...
      dataIndex: 'cover',
      width: 120,
      render: cover => { // 此处会做图片显示以及兼容处理  cover是当前列的完整数据
